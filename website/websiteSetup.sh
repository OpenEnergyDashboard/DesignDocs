...
curl localhost:3000/api/csv/readings -X POST -F 'meterName=eter 3' -F 'headerRow=true' -F 'gzip=false' -F 'email=test@example.com' -F 'password=password' -F 'csvfile=@oedData/Readings3.csv'
curl localhost:3000/api/csv/readings -X POST -F 'meterName=eter 4' -F 'headerRow=true' -F 'gzip=false' -F 'email=test@example.com' -F 'password=password' -F 'csvfile=@oedData/Readings4.csv'
