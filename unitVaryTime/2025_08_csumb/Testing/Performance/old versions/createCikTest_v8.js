...
if (require.main === module) {
	...
	if (!configPath) {
		console.error("Usage: node createCikTest_v8.js <config_file> [test_index]");
		process.exit(1);
	}
	...
	if (!Array.isArray(views) || !Array.isArray(tests)) {
		console.error("❌ Invalid config format: expected viewsToRefresh and tests arrays.");
		process.exit(1);
	}
	...
	if (testIndexArg !== undefined) {
		...
		createCikData(t.startTime, t.endTime, t.stepUnit, t.meterUnit, t.graphicUnit, t.meterId, views);
	} else {
		(async () => {
			for (const [i, t] of tests.entries()) {
				console.log(`\n🚀 Running test ${i}: ${t.startTime} → ${t.endTime} (${t.stepUnit})`);
				await createCikData(t.startTime, t.endTime, t.stepUnit, t.meterUnit, t.graphicUnit, t.meterId, views);
			}
		})();
	}
}
