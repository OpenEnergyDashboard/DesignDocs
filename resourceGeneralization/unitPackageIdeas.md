...
**This is a historical document discussing the idea of using unit packages to do the needed conversions for resource generalization.**

## issue-with-idea

The conclusion was reached that a unit package that allows bidirectional conversions will not work because it causes too many issues. Note I don't know of any unit packages that are not bidirectional and testing of mathjs found it would do the following simple example of the problem. A simple example of the issue is you have a meter in kWh that you want to display in kWh and in cost. You add a conversion for kWh to \$ so you can display money. A bidirectional unit conversion would allow you to go from \$ to kWh but we really don't want that in OED. Yes, you can buy electricity with money but that is not what the dashboard is about. Davin and I have kicked around some ideas such as limiting path lengths but here it is not a chained conversion so a path of length 1. I've found other cases where the unit conversions are causing issues in different ways. I'm open to ideas to solve this but think I have led us to the wrong solution for the given problem.
...
5. Display total energy usage as equivalent 100 watt light bulb burned for 10 hours (1 kWh) for both electricity (kWh) and gas (BTU) meters.
...
The heart of the system is allowing for conversions between compatible units. Compatible units are ones that can be aggregated because they can all be converted to the same unit and this is also logical. See [examples above](#examples) for sample conversions.
